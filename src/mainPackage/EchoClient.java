...
public class EchoClient extends Client {
    ...
    public void processMessage(String pMessage) {
        ...
        switch (nachricht[0]) {
            case "EA": mNachricht.setText(nachricht[1]); break;
            case "YOU": mClientLabyrinth.setAktPlayer(Integer.parseInt(nachricht[1])); break;
            case "L": updateLabyrinth(nachricht[1]);break;
            case "P": updatePlayer(nachricht[1]); break;
        }
    ...
    public void updatePlayer(String pNachricht){
        ...
        GraphicsPlayer[] nPlayer = new GraphicsPlayer[p.length];
        for(int i = 0 ; i < p.length ; i++){
            ...
            directions direction = directions.valueOf(att[7]);
            avatarMoods pMood = avatarMoods.valueOf(att[8]);
            String ip = att.length == 9 ? att[6]: "";// String ip = if array.length == 9 dann array[2]; wenn nicht = "";
            nPlayer[i] = new GraphicsPlayer(x, y, ip, 0, new Color(r, g, b), radius, mClientLabyrinth.getLabyrinth(), direction, pMood);
